package com.mygdx.game.tests.test6.actors;
/*
import com.badlogic.gdx.graphics.Texture;
import com.badlogic.gdx.math.Vector2;
import com.mygdx.game.cur_test.components.MyComponents;
import com.mygdx.game.ext.core.components.presets.collisionSystem.BoundingBox;
import com.mygdx.game.ext.core.components.presets.collisionSystem.CollisionType;
import com.mygdx.game.ext.core.actor.Actor;
import com.mygdx.game.ext.core.components.Component;

public class Water extends Actor
{
 public Water()
 {
  addComp(MyComponents.drawing);

  vector2 = getComponent("position"); vector2.get().x = 10;

  vector2 = getComponent("size"); vector2.get().x = 3; vector2.get().y = 3;

  addComponent("texture", new Component<>(new Texture("water.png")));

  addComponent("box", new Component<>(new BoundingBox(CollisionType.LIQUID)));

 }

 Component<Vector2> vector2;
}
*/